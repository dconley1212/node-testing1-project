...
describe("[Exercise 1] trimProperties", () => {
  ...
  test("[2] returns a copy, leaving the original object intact", () => {
    const exampleObject = { foo: "  foo ", bar: "bar ", baz: " baz" };
    const trimmedObject = utils.trimProperties(exampleObject);
    const copyOfTrimmed = { ...trimmedObject };
    expect(trimmedObject).toEqual(copyOfTrimmed);
  });
});

describe("[Exercise 2] trimPropertiesMutation", () => {
  test("[3] returns an object with the properties trimmed", () => {
    const mutationObject = { foo: "  foo ", bar: "bar ", baz: " baz" };
    const expectedMutation = { foo: "foo", bar: "bar", baz: "baz" };
